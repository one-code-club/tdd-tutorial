...
import { Terminal, CheckCircle, XCircle, Clock, Flag } from 'lucide-react'
...
export function ConsoleOutput({ messages, isExecuting = false, isReady = false, onExecute, className }: ConsoleOutputProps) {
  ...
  return (
    <Card className={cn('bg-gray-900 border-gray-700 flex flex-col', className)}>
      <CardHeader className="py-2 px-3 border-b border-gray-700 flex-shrink-0 flex flex-row items-center justify-between">
        ...
        <button
          ...
          className="w-11 h-11 rounded-full bg-green-500 hover:bg-green-400 active:bg-green-600 flex items-center justify-center disabled:opacity-50 disabled:pointer-events-none transition-all shadow-lg hover:shadow-xl active:scale-95 border-2 border-green-400/50"
          title={isExecuting ? t.console.executing : t.console.execute}
        >
          {isExecuting ? (
            <Clock className="h-5 w-5 text-white animate-spin" />
          ) : (
            <Flag className="h-6 w-6 text-white fill-white" />
          )}
        </button>
      </CardHeader>
